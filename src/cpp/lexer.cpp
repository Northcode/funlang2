...
void lexer::scan_char() {
  ...
  case READ:
    ...
    if (isdigit(current())) {
      ...
      step();
      state = STR;
    } else if (isalpha(current()) || valid_ident_chars.count(current())) {
      state = IDENT;
    } else if (current() == '?') {
      state = CHAR;
    } else if (current() == ':') {
  ...
  case IDENT:
    assert(iter != end);
    {
      ...
      if (!(isalnum(current()) || valid_ident_chars.count(current()))) {
  ...
  case PUSH:
      ...
      case t_ratio:
	{
	  ...
	  { // head is temp_buf
	    mystr temp_buf = _arena->alloc_str(buffer.len - div_index - 1);
	    strncpy(temp_buf.data, buffer.data + div_index + 1, temp_buf.len);
	    _arena->append_char(&temp_buf , '\0');
